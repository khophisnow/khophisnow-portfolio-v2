...
function normalizeRemoteState(roomId: string, incoming: GameState): GameState {
  const next = { ...createInitialState(), ...incoming, roomId };
  if (next.timerRunning && next.currentQuestion?.type === "dare" && next.timerDuration > 0) {
    return { ...next, timerEndsAt: new Date(Date.now() + next.timerDuration * 1000).toISOString(), timerExpired: false };
  }
  return next;
}

function defaultDevicePlayerId(players: Player[], role: OnlineRoom["role"]) {
...
export function TruthOrDareApp({ initialStage = "landing" }: { initialStage?: Stage } = {}) {
  ...
  const [onlineRoom, setOnlineRoom] = useState<OnlineRoom>({ enabled: false, roomId: "", status: "local", role: "local", error: "", playerId: "" });
  ...
  const devicePlayerId = onlineRoom.playerId || defaultDevicePlayerId(state.players, onlineRoom.role);
  ...
  const connectOnlineRoom = useCallback(async (roomId: string) => {
    if (!supabase) {
      setOnlineRoom({ enabled: false, roomId: "", status: "offline", role: "local", error: "Supabase is not configured for online rooms yet.", playerId: "" });
      return;
    }
    const cleanRoomId = roomId.trim();
    if (!cleanRoomId) return;
    setOnlineRoom({ enabled: false, roomId: cleanRoomId, status: "joining", role: "guest", error: "", playerId: "" });
    ...
    if (error || !data?.state) {
      setOnlineRoom({ enabled: false, roomId: cleanRoomId, status: "error", role: "guest", error: error?.message || "Online room not found.", playerId: "" });
      return;
    }
    applyingRemoteState.current = true;
    const nextState = normalizeRemoteState(cleanRoomId, data.state);
    lastOnlineState.current = JSON.stringify(nextState);
    setState(nextState);
    setOnlineRoom({ enabled: true, roomId: cleanRoomId, status: "connected", role: "guest", error: "", playerId: defaultDevicePlayerId(nextState.players, "guest") });
    window.queueMicrotask(() => { applyingRemoteState.current = false; });
  }, [supabase]);

  const createOnlineRoom = async () => {
    if (!supabase) {
      setOnlineRoom({ enabled: false, roomId: "", status: "offline", role: "local", error: "Add Supabase URL and publishable key to enable online rooms.", playerId: "" });
      return;
    }
    const roomId = id("room");
    const nextState = { ...state, roomId };
    setOnlineRoom({ enabled: false, roomId, status: "creating", role: "host", error: "", playerId: defaultDevicePlayerId(nextState.players, "host") });
    const { error } = await supabase.from("daredeck_rooms").upsert({ room_id: roomId, state: nextState, updated_at: new Date().toISOString() });
    if (error) {
      setOnlineRoom({ enabled: false, roomId, status: "error", role: "host", error: error.message, playerId: defaultDevicePlayerId(nextState.players, "host") });
      return;
    }
    lastOnlineState.current = JSON.stringify(nextState);
    setState(nextState);
    setOnlineRoom({ enabled: true, roomId, status: "connected", role: "host", error: "", playerId: defaultDevicePlayerId(nextState.players, "host") });
    window.history.replaceState(null, "", `/truth-or-dare?online=1&room=${roomId}`);
  };

  const leaveOnlineRoom = () => {
    setOnlineRoom({ enabled: false, roomId: "", status: "local", role: "local", error: "", playerId: "" });
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "daredeck_rooms", filter: `room_id=eq.${onlineRoom.roomId}` }, (payload) => {
        ...
        const nextState = normalizeRemoteState(onlineRoom.roomId, record.state);
