...
export function TruthOrDareApp({ initialStage = "landing" }: { initialStage?: Stage } = {}) {
  ...
  const markResult = (result: Result) => {
    ...
    update({ players, history: [turn, ...state.history].slice(0, 60), currentPlayerIndex: (state.currentPlayerIndex + 1) % players.length, currentQuestion: null });
...
function ArchitectureBlock() { return <section id="architecture" className="mt-14 border border-cyan/25 bg-cyan/10 p-5"><p className="font-mono text-xs uppercase text-cyan">Architecture</p><div className="mt-5 grid gap-4 md:grid-cols-5">{["Create room", "Players", "Packs", "Game engine", "Serializable state"].map((item, index) => <div key={item} className="border border-white/10 bg-black/25 p-4"><p className="font-mono text-xs text-mint">0{index + 1}</p><p className="mt-2 font-bold text-white">{item}</p></div>)}</div><p className="mt-5 text-sm leading-7 text-white/65">Storage uses localStorage for the current room, uploaded packs, score history, and exported sessions. The room ID and serialized state make the app ready for future invite-link multiplayer without adding auth or a database today.</p></section>; }
