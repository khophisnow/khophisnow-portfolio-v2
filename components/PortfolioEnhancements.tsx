...
export function AnimatedApiFlow() {
  ...
  const packetLeft = steps.length > 1 ? `${7 + active * 17.2}%` : "7%";
  return <section className="mx-auto max-w-7xl px-5 py-20 lg:px-8"><SectionTitle eyebrow="Request path" title="Watch a request pass each security checkpoint" /><div className="mt-10 border border-white/10 bg-panel/70 p-5"><div className="hidden md:block"><div className="grid grid-cols-6 gap-4">{steps.map((step, index) => { const Icon = step.icon; return <article key={step.label} className={`min-h-[150px] border p-4 text-center transition ${active === index ? "flow-card-active" : "border-white/10 bg-black/35"}`}><div className={`mx-auto flex size-12 items-center justify-center border transition ${active === index ? "flow-node-active border-transparent" : "border-white/15 bg-white/[0.035] text-white/55"}`}><Icon size={21} /></div><p className="mt-4 text-sm font-black text-white">{step.label}</p><p className="mt-2 text-xs text-mint">{step.visual}</p></article>; })}</div><div className="request-rail relative mt-6 h-12"><div className="absolute left-3 right-3 top-1/2 h-1 -translate-y-1/2 rounded-full bg-gradient-to-r from-mint/15 via-mint/70 to-cyan/25" /><div className="absolute left-3 right-3 top-1/2 h-px -translate-y-1/2 bg-white/18" /><span style={{ left: packetLeft }} className="request-runner absolute top-1/2 -translate-x-1/2 -translate-y-1/2 transition-[left] duration-[1200ms] ease-in-out"><span className="request-runner-trail" /><span className="request-runner-car"><Send size={13} /></span></span></div></div><div className="grid gap-3 md:hidden">{steps.map((step, index) => { const Icon = step.icon; return <article key={step.label} className={`flex items-center gap-3 border p-4 text-left ${active === index ? "flow-card-active" : "border-white/10 bg-black/35"}`}><span className={`flex size-11 shrink-0 items-center justify-center border ${active === index ? "flow-node-active" : "border-white/15 text-white/55"}`}><Icon size={19} /></span><span><span className="block font-bold text-white">{step.label}</span><span className="mt-1 block text-xs text-white/55">{step.visual}</span></span></article>; })}</div><div className="mt-5 border border-white/10 bg-black/30 p-5"><p className="font-bold text-white">{steps[active].label}: <span className="text-mint">{steps[active].visual}</span></p><p className="mt-3 max-w-3xl text-sm leading-7 text-white/68">{steps[active].detail}</p><div className="mt-4 flex flex-wrap gap-2">{steps[active].checks.map((check) => <span key={check} className="border border-mint/35 bg-mint/10 px-3 py-1 text-xs text-white/75">{check}</span>)}</div></div></div></section>;
}
