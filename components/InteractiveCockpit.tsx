...
export function ApiSimulator() {
  ...
  return <div className="min-w-0 border border-white/10 bg-panel/75 p-4 sm:p-5"><div className="flex min-w-0 items-center gap-2 text-mint"><Terminal size={18} className="shrink-0" /><p className="min-w-0 break-words font-mono text-xs uppercase">API request simulator</p></div><div className="mt-5 flex min-w-0 flex-wrap gap-2">{endpoints.map((item) => <button key={item} type="button" onClick={() => setEndpoint(item)} className={`max-w-full break-all border px-3 py-2 font-mono text-xs transition ${endpoint === item ? "border-mint bg-mint text-ink" : "border-white/10 bg-white/[0.035] text-white/68 hover:border-cyan"}`}>GET {item}</button>)}</div><pre className="mt-5 max-h-[360px] max-w-full overflow-x-auto whitespace-pre-wrap break-words border border-white/10 bg-black/40 p-3 text-xs leading-6 text-cyan sm:p-4"><code className="break-words">{response}</code></pre></div>;
}
